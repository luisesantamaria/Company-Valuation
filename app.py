...
with st.spinner("Loading company info…"):
    ...
    comp_industry = (info.get("industry") or "").strip() or "—"
    yf_sector = (info.get("sector") or "").strip()
    if not comp_sector or comp_sector.lower() in {"", "nan", "none"}:
        comp_sector = yf_sector or comp_sector or "—"
...
with col1:
    ...
    st.caption(f"Industry: {comp_industry}")
with col2:
